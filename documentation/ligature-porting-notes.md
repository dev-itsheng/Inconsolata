...
| `===` 看起来像两条横线，且和 `==` 垂直中心不一致 | 三字符等号在小字号和 overview 里与 `==` 不好区分；放在一起时 `==` 像是更靠上 | 两字符 equality 和三字符 identity 是不同语义，不能共用两横线 run 逻辑；同时 `==` 应继承原始 `equal` 的位置，不应为了对齐被下移 | 单独用 `equal_triple` 生成三条等长横线，并把三横线整体视觉中心对齐到原始 `equal` / `==`；`====` / `=====` 以及更长连续等号才走两条连续分割线逻辑 |
| `<=` / `>=` 容易被读成箭头逻辑，第二条斜线也容易错位 | 和 `<==` / `==>` 放在同一组时，用户会把比较符号误读成方向符号；如果改成水平 bar，又会偏离 Fira Code 默认风格；只看 bbox 居中会漏掉端点不齐 | `<=` / `>=` 是“小于等于 / 大于等于”，不是 `=` 箭头族；Fira Code 默认是两条平行斜线，水平 bar 是可选变体；这类 glyph 要审计端点相位，不只审计左右 margin | 用 `comparison_equal` 按 `<` / `>` 下斜边的真实视觉跨度生成同源、平行、等长、两端竖切的第二条斜线，并把 overview / catalog 分组挪到 comparisons |
...
| `/*` / `*/` 会误伤 glob 模式 | `**/*.{ts,tsx}`、`!**/composables/**/*.ts` 里既有 globstar `**`，也有 `/` 和 `*` 相邻 | 如果把 `/*` / `*/` 当成无条件固定 `liga` / `dlig`，glob 里会出现“像块注释一样”的视觉合并；这会让路径模式比源码更难扫读 | 保留 `slash_asterisk.dlig` / `asterisk_slash.dlig` glyph，但从无条件 `liga` / `dlig` 中排除；改用 `calt` spacer 两步规则，`/*` 只在后接普通空格时触发，`*/` 只在前接普通空格时触发。`/* block */` 正常显示块注释边界，glob / path 场景保持 raw `/*` / `*/`，但 `**` 仍作为 globstar 连写 |
